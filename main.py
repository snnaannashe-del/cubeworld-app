...
async def get_cube_online_members(cube_id: int, user=Depends(get_current_user)):
    ...
    try:
        ...
        pass  # safe fallback Ã¢ÂÂ live users are already in seen
...
async def like_comment(post_id: int, cid: int, body: dict, user=Depends(get_current_user)):
    like_type = body.get("type", "like")
    if like_type not in ("like", "dislike"):
        raise HTTPException(400, "Invalid type")
    return db.toggle_comment_like(cid, user["id"], like_type)

@app.delete("/feed/{post_id}/comment/{cid}")
async def delete_comment(post_id: int, cid: int, user=Depends(get_current_user)):
    deleted = db.delete_post_comment(cid, user["id"])
    if not deleted:
        raise HTTPException(403, "Not authorized or not found")
    return {"ok": True}

@app.put("/feed/{post_id}/comment/{cid}")
async def edit_comment(post_id: int, cid: int, body: AddCommentRequest, user=Depends(get_current_user)):
    content = (body.content or "").strip()[:500]
    if not content:
        raise HTTPException(400, "Content required")
    updated = db.edit_post_comment(cid, user["id"], content)
    if not updated:
        raise HTTPException(403, "Not authorized or not found")
...
@app.get("/health")
async def health():
    return {"status": "ok", "version": "5.3.0", "kick_route": "active", "cube_visitors": "active"}
...
async def upload_video(file: UploadFile = File(...)):
    ...
    if ext.lower() not in (".mp4", ".webm", ".mov", ".avi", ".mkv", ".3gp"):
        ext = ".mp4"
    filename = "v_" + str(int(_time.time())) + "_" + uuid.uuid4().hex[:8] + ext
    ...
    return {"url": "https://cubeworld-backend.onrender.com/uploads/" + filename}
