...
def init_db():
    ...
    if _PG:
        ...
        # Add expire_at columns if missing (migration)
        try:
            c.execute("ALTER TABLE post_comments ADD COLUMN expire_at TIMESTAMP")
            conn.commit()
        except Exception: pass
        try:
            c.execute("ALTER TABLE comment_replies ADD COLUMN expire_at TIMESTAMP")
            conn.commit()
        except Exception: pass
...
def get_global_feed(limit=30, offset=0):
    ...
    sql = _q("SELECT p.*, u.avatar_url FROM posts p LEFT JOIN users u ON u.id=p.user_id ORDER BY p.created_at DESC LIMIT ? OFFSET ?")
...
def get_post_comments(post_id, limit=100):
    conn = get_db(); c = conn.cursor()
    sql = _q("""SELECT pc.id, pc.post_id, pc.user_id, pc.display_name, pc.content, pc.created_at,
               COALESCE(SUM(CASE WHEN cl.type='like' THEN 1 ELSE 0 END),0) as likes,
               COALESCE(SUM(CASE WHEN cl.type='dislike' THEN 1 ELSE 0 END),0) as dislikes
               FROM post_comments pc
               LEFT JOIN comment_likes cl ON pc.id=cl.comment_id
               WHERE pc.post_id=?
               GROUP BY pc.id, pc.post_id, pc.user_id, pc.display_name, pc.content, pc.created_at
               ORDER BY pc.created_at ASC LIMIT ?""")
    c.execute(sql, (post_id, limit))
...
def toggle_comment_like(comment_id, user_id, like_type):
    ...
    try:
        ...
        existing = c.fetchone()
        if existing:
            if (existing[0] if not hasattr(existing, "keys") else existing["type"]) == like_type:
...
def delete_post_comment(comment_id, user_id):
    ...
    try:
        c = conn.cursor()
        c.execute(_q("DELETE FROM post_comments WHERE id=? AND user_id=?"), (comment_id, user_id))
        deleted = c.rowcount > 0
        conn.commit()
        return deleted
    finally:
...
def edit_post_comment(comment_id, user_id, content):
    ...
    try:
        c = conn.cursor()
        c.execute(_q("UPDATE post_comments SET content=? WHERE id=? AND user_id=?"), (content, comment_id, user_id))
...
def add_comment_reply(comment_id, user_id, display_name, content, expire_seconds=None):
    ...
    try:
        ...
        if expire_seconds:
            ...
            # Inherit parent comment's expire_at
